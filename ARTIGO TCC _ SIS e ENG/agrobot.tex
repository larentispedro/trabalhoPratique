...
O Diagrama de Sequência referente ao caso de uso \textit{Publicar Anúncio} descreve o fluxo de interação entre o Agricultor, o AgroBot e o Banco de Dados. Esse processo representa uma das funcionalidades centrais do sistema, uma vez que a criação de anúncios é o que alimenta o marketplace agrícola dentro do chatbot.
...
\textit{Listar e Filtrar Anúncios}, que corresponde a um dos processos mais utilizados pelos
...
O Diagrama de Atividades referente ao caso de uso \textit{Publicar Anúncio} descreve o fluxo operacional da criação de um novo anúncio dentro do AgroBot. Esse diagrama representa, de forma estruturada, as ações realizadas pelo usuário, as respostas automáticas do bot e o processamento interno do sistema, evidenciando a colaboração entre as diferentes partes envolvidas no processo.

O fluxo inicia na raia do \textbf{Usuário}, que seleciona a opção de publicar um anúncio. A partir desse ponto, o controle é alternado entre o usuário e o \textbf{AgroBot}, que conduz a interação de maneira sequencial e guiada. O bot solicita primeiro a categoria do anúncio, seguida da descrição e da localização. Cada solicitação é respondida pelo usuário, caracterizando um fluxo linear e fácil de seguir — algo especialmente importante para agricultores com pouca familiaridade tecnológica.

Após coletar todas as informações necessárias, o AgroBot transfere o processamento para a raia do \textbf{Banco de Dados}, enviando os dados coletados para que o anúncio seja registrado. A base de dados cria a nova entrada e retorna uma confirmação ao bot.
